...
import {
  ...
  HiOutlineUserGroup, HiChatAlt2, HiUserCircle, HiPlusCircle,
} from 'react-icons/hi';
...
export default function PembeliPage() {
  ...
  const pembeli = [
    { name: 'PT Grow Garden', totalTransaksi: 12, totalBelanja: 'Rp 8.300.000', status: 'Aktif' },
    { name: 'Toko Pakan "Sejahtera"', totalTransaksi: 5, totalBelanja: 'Rp 2.100.000', status: 'Aktif' },
    { name: 'Bu Sinta', totalTransaksi: 3, totalBelanja: 'Rp 600.000', status: 'Aktif' },
  ];
            ...
            <div style={{ padding: '2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                <div style={{ width: 56, height: 56, borderRadius: 16, background: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <HiOutlineUserGroup style={{ color: '#15803d', fontSize: '2rem' }} />
                </div>
                <div>
                  <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: '#1f2937', margin: 0 }}>Pembeli Aktif</h1>
                  <p style={{ color: '#6b7280', marginTop: '0.25rem' }}>Daftar pembeli yang sering bertransaksi dengan Anda</p>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {pembeli.map((item, i) => (
                  <div
                    key={i}
                    onClick={() => router.push(`/petani/pembeli/${item.name.toLowerCase().replace(/\s/g, '-')}`)}
                    style={{
                  ...
                  >
                    <div style={{ flex: '2', minWidth: '180px' }}>
                      <p style={{ fontWeight: 700, fontSize: '1rem', color: '#1f2937', margin: 0 }}>{item.name}</p>
                      <p style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '0.25rem' }}>{item.totalTransaksi} transaksi</p>
                    </div>
                    <div style={{ minWidth: '120px' }}>
                      <p style={{ fontWeight: 600, fontSize: '1rem', color: '#15803d', margin: 0 }}>{item.totalBelanja}</p>
                    </div>
                    <div>
                      <span style={{
